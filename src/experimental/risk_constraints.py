"""Experimental risk constraints for portfolio construction.
...
These constraints are applied AFTER signal generation but BEFORE passing to a
backtest executor. They modify prediction scores to enforce constraints
implicitly through the TopkDropout strategy and are therefore runtime trading
behavior.

Boundaries
----------
- Explicitly experimental; not an official/canonical metrics path.
- Requires canonical qlib init only to keep qlib-global-state assumptions
  visible during experiments.
