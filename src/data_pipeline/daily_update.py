...
- fetch runs ``01_fetch_tushare --refresh-current`` so the AGGREGATE units a
  ...
  daily_basic) use content freshness instead: a scanned year file with expected
  sessions must have valid same-year dates spanning both expected boundaries.
  Short or invalid files select a refetch subject to the overwrite guard;
  a same-day crash re-run skips already-current files.
