...
Identify whether the change is foundation, contract, runtime, UI, test, or documentation work.
Check whether OpenSpec approval is required.
Inspect only the files needed for the scoped change.
Implement the smallest safe change.
Add or update targeted tests when behavior or governance boundaries are affected.
Run the narrowest useful validation first; expand only when needed.
Summarize changed files, scope, tests, and any intentionally unimplemented work.
Change categories
Foundation changes
Allowed:
Skeletons.
