...
| `QUANT_ENSEMBLE_MANIFEST` | operator UI ONLY (今日推荐 现任身份横幅 + ensemble 工件交叉核对) | *(unset — 未设即视为现任仍是单模型形态)* | Points the UI at the **incumbent** production ensemble manifest so the page can say WHICH model is serving and whether a shown artifact came from it. **Read-side only, on purpose**: `scripts/daily_recommend.py --ensemble-manifest` stays explicitly required and MUST NOT inherit this default — the side that PRODUCES a list must never pick its model implicitly (a wrong pick is a wrong order list); the side that only CROSS-CHECKS may. Set but unreadable → the page WARNs; it never falls back to the single-model banner. |
