...
| `QUANT_FIN_STORE` | `config/factor_mining/fundamental_link_check.yaml` (`data.fundamental_store_root`) | `D:/qlib_data/financial_pit_raw` | Financial-statement PIT store (income/balancesheet/cashflow parquets) the fundamental panel factory reads; content-fingerprinted twice per mining run and twice per promotion. |
| `QUANT_FIN_CALENDAR` | `config/factor_mining/fundamental_link_check.yaml` (`data.fundamental_calendar_path`) | `D:/qlib_data/my_cn_data_pit/calendars/day.txt` | Trading-calendar file (one ISO date per line) the financial PIT view slices availability against; fingerprinted together with the store. |
| `QUANT_DECISION_JOURNAL_DIR` | `web/operator_ui/decision_journal.py` (日度信号与人工决策的 decision journal) | `D:/stock/operator_journal` | Append-only operator decision journal (JSONL). Lives OUTSIDE the repo's disposable `output/` tree; web-layer state — NEVER an input to official metrics / backtests / training (src/ zero-reference is test-enforced). |
