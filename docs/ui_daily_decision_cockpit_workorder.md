...
3. **out-dir 必须锁定 canonical 工件根**（r2）：查看器只枚举
   ...
   移除。
4. **同页 run 表单与只读契约冲突须显式解**（r3）：`v2-daily-decision-page`
   规格明确要求该页**绝不触发作业**，且源码守卫测试禁止页内出现
   `JobManager` / `job_runner` / `subprocess`——§4.1"同页表单/新 tab"与
   §4.2"不改只读契约"不可同时成立。P1 的 OpenSpec 必须二选一并写明：
   (i) 显式 REPLACE 该 never-triggers-jobs 需求 + 同步改源码守卫测试；
