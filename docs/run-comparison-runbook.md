...
| The two runs' ST handling disagrees — `st_mask_mode`, the CONCRETE `namechange_path` (separator-normalized), or the recorded input CONTENT hash (per-fold `backtest.provenance.config.st_mask.namechange_sha256`) | ST-on vs ST-off measures the PR#223 ST interaction; a different snapshot — including the SAME path refreshed in place between the runs — excludes a different ST set. Either way the pair measures an input change, not the registered hypothesis. "Same ST handling on both sides" must hold by machine check, not by care. Re-run the mismatched side. |
| An ST-on run whose fold reports carry no `namechange_sha256` content hash (or record several distinct ones) | Path-only proof is no proof — the file can be refreshed in place; several distinct hashes mean the input moved MID-RUN. Re-run on the current engine in one uninterrupted invocation. |
