...
The `@codex` PR review should CONFIRM a clean change, not DISCOVER problems. Before you open or update a PR, run the `qlib-code-review` skill against your own diff and iterate to convergence: review (`docs/codex/code-review.md`, P0–P3) → fix P0/P1 → re-review → repeat until the reviewed scope is clean (or only accepted P3 remain). A six-round PR review is the signal this loop was skipped — the review work happened on slow remote round-trips instead of fast local ones. Runbook: `docs/codex/local-review-loop.md`. This loop does NOT replace the mechanical-move whole-file content-diff proof above, nor the deterministic gates in `docs/codex/review-findings-to-harden.md` — they catch different things; run them too.
Pre-commit hook
