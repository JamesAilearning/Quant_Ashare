...
from web.operator_ui.report_reader import read_walk_forward_report, read_fold_reports
    ...
    if fold_reports:
        ...
        for fr in fold_reports:
            with st.expander(f"Fold {fr.get('fold_index', '?')}"):
                ...
                cols = st.columns(4)
                cols[0].metric("IC(1d)", f"{metrics.get('ic_1d', 0):.4f}")
                cols[1].metric("IC(5d)", f"{metrics.get('ic_5d', 0):.4f}")
                cols[2].metric("Return", f"{metrics.get('annualized_return', 0):.4f}")
                cols[3].metric("IR", f"{metrics.get('information_ratio', 0):.4f}")
