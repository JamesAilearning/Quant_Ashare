...
The pure helpers (artifact reading, format, status, path-guard,
JSON-filter, chart/frame readers, job-list helpers) live in
``pages/_results_helpers.py`` and are re-exported here so external
tests keep working with ``from web.operator_ui.pages.results import
_filter_json_by_query`` and friends. This module is now scoped to the
Streamlit-dispatching surface: ``_render_*`` functions and the
module-level page entry. UI review P1-1, phase 1 (PR-J1).
"""
...
# Re-export pure helpers + module constants from the helpers module.
# These names stay attribute-accessible on ``web.operator_ui.pages.results``
# so existing tests (``from web.operator_ui.pages.results import
# _filter_json_by_query`` etc.) keep working.
