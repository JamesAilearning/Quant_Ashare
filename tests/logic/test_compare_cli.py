...
def _write_run(root: Path, dates: list[str], excess: np.ndarray[Any, Any],
    ...
    (root / "fold_00_report.json").write_text(json.dumps({
        ...
        # (codex P1 #323 r3) — nested exactly as a real run writes it:
        # backtest.provenance.config.st_mask (BacktestRunner flattens the
        # strategy dict into provenance's config sub-mapping; smoke-run
        # verified after r4's guess landed one level short).
        "backtest": {"provenance": {"config": {"st_mask": {
