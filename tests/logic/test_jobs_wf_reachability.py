...
class PageSourcePinsTests(unittest.TestCase):
    ...
    def test_jobs_page_discloses_skipped_cli_catalog_records(self) -> None:
        src = _PAGE_JOBS.read_text(encoding="utf-8")
        self.assertIn("count_malformed_cli_entries", src)
        self.assertIn("CLI 目录索引记录未列出", src)
        disclosure_at = src.index("_malformed_cli = count_malformed_cli_entries()")
        first_empty_at = src.index("if total == 0 and not _active:")
        self.assertLess(disclosure_at, first_empty_at)
