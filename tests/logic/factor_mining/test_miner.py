...
def test_run_dir_is_never_silently_overwritten(tmp_path):
    # External finding #3 (2026-08-10): a pinned run_id re-run used to
    # exist_ok=True its way into REPLACING the earlier run's pool / history
    # / config — replaceable provenance for whatever later got promoted.
    cfg = load_config(_smoke_config(tmp_path))
    run_mining(cfg)
    with pytest.raises(RuntimeError, match="already exists"):
        run_mining(cfg)  # same pinned run_id -> same dir -> refuse


def test_duplicate_run_refused_before_mining_starts(tmp_path):
    # codex P2 #418: the refusal must come BEFORE the expensive part — a
    # duplicate real-PIT campaign must not burn the whole GP run first.
    ...
    cfg = load_config(_smoke_config(tmp_path))
    run_mining(cfg)
    with patch.object(miner_mod, "build_panel",
                      side_effect=AssertionError("mining started")) as fake:
        with pytest.raises(RuntimeError, match="already exists"):
            run_mining(cfg)
...
def test_failed_run_releases_the_reservation(tmp_path):
    # A run that dies mid-mining must not leave an empty reserved dir
    # blocking the retry of the same pinned run_id.
    from unittest.mock import patch

    from src.factor_mining import miner as miner_mod

    cfg = load_config(_smoke_config(tmp_path))
    run_dir = Path(cfg.output_dir) / "runs" / cfg.run_id
    with patch.object(miner_mod.GPEngine, "run",
                      side_effect=RuntimeError("boom")):
        with pytest.raises(RuntimeError, match="boom"):
            run_mining(cfg)
    assert not run_dir.exists(), "empty reservation must be released"
    # ...and the retry now proceeds normally.
    result = run_mining(cfg)
    assert (result.output_dir / "factor_pool.parquet").is_file()


def test_autogenerated_run_ids_are_unique_same_second(tmp_path):
    # Timestamp+seed alone collides within one second; the random suffix
    # makes every invocation unique by construction.
    from src.factor_mining.miner import _autogenerate_run_id

    ids = {_autogenerate_run_id(7) for _ in range(64)}
    assert len(ids) == 64
    for run_id in ids:
        assert "-7-" in run_id  # stamp-seed-suffix shape retained


def test_autogenerated_runs_do_not_collide(tmp_path):
    # Two same-seed launches with run_id null must land in DIFFERENT dirs.
    cfg1 = load_config(_smoke_config(tmp_path))
    cfg1 = MinerConfig(
        data=cfg1.data, gp=cfg1.gp, fitness=cfg1.fitness,
        output_dir=cfg1.output_dir, run_id=None,
        pool_top_k=cfg1.pool_top_k,
    )
    r1 = run_mining(cfg1)
    r2 = run_mining(cfg1)
    assert r1.output_dir != r2.output_dir
    assert (r1.output_dir / "factor_pool.parquet").is_file()
    assert (r2.output_dir / "factor_pool.parquet").is_file()
