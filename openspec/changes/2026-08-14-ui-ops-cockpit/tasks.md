...
## W36 codex #431 r30（**P1**，属实；推翻了我 r28 写下的前提）
- [x] 我 r28 的注释写「在错误宿主上它会直接读不到」——**是错的**。
      POSIX 没有盘符,`D:/…` 被当**相对**路径按各自 CWD 解析。实测:
      Streamlit 起于 `/tmp` 读 `/tmp/D:/qlib_data/…`,命令起于仓库根读
      `<checkout>/D:/qlib_data/…` —— r27 的分歧换个写法原样回来
- [x] 新增 `foreign_absolute_reason()`:仅当「另一套约定下绝对、本机约定下相对」
      时非空  ← C93 咬住
- [x] 命令侧由**既有边界** `_arg` 统一拒绝(三个构造器不需各自记得)  ← C94 咬住
- [x] 两道读取器同样拒绝,不去读 `<cwd>/D:/…`  ← C95/C96 咬住
- [x] **不**锚到仓库根:那样页面与命令确实一致了,却一致地指向一个不存在的
      位置,会把操作人引去查「bundle 丢了」而不是「路径配错了」  ← C97 咬住
- [x] 页面顶部一次性说明病因  ← C98 咬住
- [x] Windows(文档化平台)上行为**完全不变** —— 用 mock isabs 双向对拍钉住
