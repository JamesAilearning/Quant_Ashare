...
- [x] 新守卫 50 条 + 49 subtests

## codex 第二轮：一个 P1 + 一个 P2

**P1 规格自相矛盾**：我把范围控制写成了 ADDED，而基线里那条
`数据更新手动启动为 detached 子进程` 仍然钉死 `--start-date 20180101` 的固定
argv。归档之后基线里会同时存在两条互相矛盾的契约，实现无法同时满足。改为
**MODIFIED**：把基线那条整条重写，缺省逐字相同、偏离来自显式输入，预检加第
(d) 条（范围）。只能有一条权威——这正是这个仓库反复付学费的那件事。

**P2 全角数字**：Python 的 `\d` 与 `int()` 都收 Unicode 数字，粘贴进来的
`２０２７０１０１` 能过形状检查、也能构造出 date；而顺序比较按字典序比原串，
全角码位远在 ASCII 之上——一个数值上更早的结束日期会被判成更晚，颠倒的区间
就这样交给了子进程。形状检查改为 ASCII-only 的 `[0-9]`。
- [x] 变异 21 条全咬（argv 夹带与空开始日期两条**同时**被既有守卫抓到）

## codex 第一轮：三条 P2，全部属实

| | 问题 |
|---|---|
| P2 | 启动修复跑在日历闸**之前**：live 没了但 `.bak` 还在时会被恢复，闸随后照样 no-op，而我查的是修复**前**的状态 → 漏报 |
| P2 | 结束日期留空 = 运行日，只比字面输入时 `start=20270101` 会通过校验 |
| P2 | 开始日期留空 = `START_DATE`，只比字面输入时 `end=20170101` 会通过校验 |

后两条是同一个根：**比的是字面输入，不是生效值**。留空不等于「没有这一端」。

第一条要复现的是 `check_and_repair` 之后的状态。`.new` / `.bak` 的命名与修复
语义同样是 UI 侧的重述，所以拿真的 `check_and_repair` 当 oracle 穷尽比一遍。

## 「变异没抓到」的处置：夹具太齐整

把 `.bak` / `.new` 的优先级弄反，第一版**抓不到**——因为夹具里两个兄弟目录都是
完整骨架，「看哪一个」没有区别。而 `check_and_repair` 在两者都在时提升的是
`.new`，只有让 `.new` 残缺、`.bak` 完整，这条优先级才显形。

夹具从「在/不在」改成**三态**（完整 / 残缺 / 不存在），3³ = 27 种组合全跑，
外加把那格最容易忽略的单独拎出来写了一条。守卫弱不弱，有时不在断言而在夹具。
