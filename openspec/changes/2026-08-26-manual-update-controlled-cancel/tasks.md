...
## codex 第三十九轮（P2：legacy 迟到终录钉生存期候选）

- [x] P2 补结算的 legacy 终录检出用观测上界——watcher 观测可晚于真实死
      亡最长一个轮询周期,回收 pid 的接替者在冻结/粗粒度时钟下可写出仍
      落窗的 finished,接替者的终态被归给被取消运行。修=与 r12 收养同
      构:legacy（无 kill nonce）时终录归属钉在生存期内观察到的精确戳
      候选上（终态记录与 running 共享 started_at,producer base 字典保
...
## codex 第四十一轮（P2：legacy 迟到终录归属 fail-closed——r39/40 改判收口）

- [x] P2 codex 点破数学终点:精确戳/生存期候选在冻结时钟下同样可被同
      pid 同戳接替**重放**——legacy 路径里 pid 可回收、一切时间量可重
      放,不存在不可重放身份。r39（结算帧候选钉）/r40（渲染帧精确戳）
      的逐层加固到此改判:**无 nonce 即不做迟到终录归属**
      （_late_terminal fail-closed）,孤儿走收养链/陈旧线,页面按工件如
...
## codex 第四十五轮（P2：watcher 盯**任何**在飞句柄,不限未决取消）

- [x] P2 watcher 只盯未决取消句柄（r9 P2a 的窄口径）——句柄死亡不改签
      名不推进度的情形不止硬杀一种:普通在飞运行被外部杀死而没写终录、
      终录已写而子进程仍在收尾台账,两种下顶部退役块都要等操作人手动
      交互才执行,取消控件与被禁的启动按钮挂到六小时陈旧线。修=
      `_pending_cancel_proc` → `_watched_live_proc = _session_live_proc`
      （顶部退役块已把死句柄清成 None,这里必是活句柄或 None）,watching
      与片段死亡检测同步扩面
- [x] **变异逃逸一次**（只钉名字时,把赋值改回「仅未决取消」的条件表达
      式仍能过）——钉收紧为**整行无条件赋值**后咬住。教训:钉标识符不
      等于钉语义,条件收窄类变异要钉表达式本体

## codex 第四十六轮（P2：普通退役也要落中断证据）

- [x] P2 普通退役分支（无未决取消）丢句柄时不落任何证据——子进程可能
      写了 nonce-bearing 的 running 记录却没写终录就被外部杀死/崩溃,那
      条记录无人更正,`_running_fresh` 恒真,启动按钮锁到六小时陈旧线
      （r45 扩面后 watcher 能自动到达该分支,但分支本身没解闸,等于自动
      化了一个无效退役）。修=退役前落**中断**证据（同键,
      `kind: "interrupted"`,身份=本会话先验已知的 nonce;惰性,无匹配
      记录时无害）;running 分支措辞按 kind 分岔——中断**不许**说成
      「已被本会话取消」（运行死于外部原因,页面只知道句柄确认退出）。
      四处取消证据同步补 `kind: "cancelled"`
- [x] **变异再逃逸一次**（钉赋值行时,条件熄火成 `if False and ...` 仍
      过）——钉收紧到**条件行本身**后咬住。与 r45 同款教训,已连续两轮
      印证:条件收窄/熄火类变异必须钉条件表达式所在整行

## 终轮划线（2026-08-27 用户裁决，#463 先例）

codex 循环已 46 轮 / 58 条意见。用户裁决**划线**：本轮（head
`2ac51b2`）返回的意见**不再逐条修**，原样记档如下；#470 转入等 merge
状态。划线依据（呈裁决时的实测归因）:

* 46 轮里 **12 轮**耗在同一个问题上（「什么身份是可证的」四代改判:
  请求上界→观测上界→生存期候选→launch nonce）——本可在 r9 一次问清
  「什么身份构造性不可重放」,是**我的方法问题**（逐层加固 vs 修在
  不变式层）,不是意见质量问题
* ~7 轮是我的修复引入的链式后果（r13←r9、r28←r27、r40←r39、r46←r45）
* 3 处是我自己的论证/操作错误（r43 数错入口数、r44 误读 signal_issued
  语义、连续两轮变异逃逸）
* nonce 主路径自 r24 起构造性正确,legacy 面 r41-r44 已整体
  fail-closed 删除;近几轮意见集中在链式后果与呈报措辞,边际收益递减

### 终轮遗留（按裁决记档不修）

**P2「中断证据落盘后没有重绘」**（head `2ac51b2`,
`web/operator_ui/pages/run_center.py:395` 附近）:普通退役分支落
`kind: "interrupted"` 证据时,`_cancelled_this_run` / `_running_fresh`
已在本帧更早处算定,而该分支清掉句柄后既不 rerun 也不重算——本帧仍把
那条 running 记录呈现为活、启动按钮仍禁用;句柄已清 ⇒ fragment 不再
触发重绘 ⇒ 要等操作人手动交互或六小时陈旧线。

* **后果**（不修的实际影响）:r46 的中断证据要**手动刷新一次**才显
  现。不是新缺陷,是退回 r45 之前的老行为（普通退役后需手动刷一次）
* **修法（若日后重开）**:落证据后 `st.rerun()`,与补结算分支
  （`_settle_late_pending` 末尾）同款;codex 另要求把该顺序做成**可执
  行**回归而非源码串断言——那需要给页面主脚本引入可测的帧模拟层,是
  独立议题
* codex 原文见 PR #470 inline comment（head `2ac51b2`）
