...
- **veto④ 校准修订（选项 A，2026-07-17 操作人签）**：首发点火（csi300
  参照）被 P0-1 默认约束击落 23/23 折——`max_per_board` 0.40 与
  `cash_buffer_min` 0.01 对长仓指数篮子/满仓 topk 策略是**结构性不
  兼容**（board 桶非风险行业；实证沪主板 53-60%、现金 0.55-0.9%），
  该引擎自建成即 dormant、默认值从未对真实策略验证。修订于零战役
  结果窗口内：max_per_name 0.05 + max_leverage 1.0 严格保留（真有
  牙齿且实证通过），board/cash 两项以理由入档禁用
  （`campaign_risk_constraints_v1`，治理 pin）。
