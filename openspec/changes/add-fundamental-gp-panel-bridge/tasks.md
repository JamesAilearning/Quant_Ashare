...
- [x] `src/research/financial_pit_view.py`：`as_of` 增加**公开的 provenance 响应**
      ...
      （证据记的是"服务了哪条披露"，不是"值是否有效"；证据为 NA 的条件是**该日
      没有任何记录被服务**，即尚无任何期**可用** —— **不是**"尚无任何已公告记录"：
      可用日严格晚于公告日，所以公告日当天记录已公告却无物可服务，证据同样必须为
      NA，codex #427 r24 P1）——否则"两期皆 NA"的字段在公告日平移下值与证据都不变，
      平移诊断会误拒正确实现（codex #427 r3 P2）。
- [x] `src/factor_mining/grammar.py`：注册**基本面终端组** + 其类型/taint 规则。
...
- [x] **终端名 ↔ charter 字段名的映射**（codex #427 r4 P1）：注册终端还不够 ——
...
- [x] **prior 期做成一等公民**（codex #427 r4 P1）：`__prior` 只挂在面板旁边是**长不出
...
- [x] `src/research/fundamental_panel.py`：`build_fundamental_panel(view, fields,
...
- [x] 返回前自检：`available_from <= trade_date`，**键在证据上而非值上**
...
- [x] **跨期 provenance**：面板携带每个字段的 `report_period`
      （用 `include_report_periods`）与差分类字段的 `__prior` 及其自身 provenance
      （用 `include_prior_period`）。
- [x] **命名空间冻结**（codex #427 r2 P1）：面板 instrument 标签必须与 GP 面板
...
- [x] **跨端点同期强制放到 expression-aware 层**（codex #427 r2 P1）：面板层做不到 ——
...
- [x] **把 period provenance 接进求值路径**（codex #427 r3 P1）：桥把 `periods` 作为
...
- [x] **接线要覆盖每一个求值调用点，不止搜索路径**（codex #427 r4 P1）：
      ...
      迁移调用方与测试」）。
- [x] **晋升入口也要接**（codex #427 r5 P1）：`promote.py:376` 的
      ...
      并有端到端晋升测试。
- [x] **注入缝：面板构造器由脚本层传进来，不由 `src/factor_mining/` 导入**
      ...
      **没有可实现路径**：导入即破闸，不导入则根本造不出面板。
      解法 = `run_mining` 与 `promote_run` 各接一个**可注入的面板工厂**（数据仍全部
      来自 run 持久化契约，**工厂本身**是唯一例外 —— 它由缝进来、由记录的身份来绑，
      不传时行为
      ...
      **但②在扩窗晋升下不够**（codex #427 r15 P1）：`promote.py:357-363` 允许
      `validation_end_date` 扩窗，晋升会评估挖掘 run 从未覆盖的日期，而按**原窗**
      重算的输出摘要对那些日期一无所知 —— 被调包的 callable 可以在原窗上逐位复现
      值/证据/periods，却在新增的 OOS 日期上发散，恰恰是扩窗要裁决的那段。扩窗
      晋升须由**二者之一**绑定（codex #427 r16/r17 P1）：①实现/依赖摘要，或
      ②'由**独立可信过程**在晋升前落盘的预期有效窗输出摘要'。被排除的是第三种 ——
      **基线由被晋升的工厂自己产出**。理由：
      `validation_end_date` 只在晋升时给出，挖掘只可能存下**原窗**的输出摘要 ——
      拿有效窗摘要去比它必然不等，而去哈希"被晋升工厂自己在有效窗上的输出"
      等于让被调包的 callable **自己给自己发证**。两条可走的路都成立，因为它们的
      基线都来自晋升**不曾产出**的地方。**注意扩窗下的输出摘要基线不能来自挖掘**
      （codex #427 r18 P1）：挖掘只可能覆盖原窗，拿它当扩窗基线要么放过新增 OOS
      日期、要么必然不等 —— 扩窗的输出摘要基线**只能**是那份独立可信的预授权基线。
- [x] **金融排除要一并进宇宙掩码**（codex #427 r12 P1）：`run_mining` 另行取
...
- [x] **`DataConfig` 要记全重建面板的输入**（codex #427 r6 P1）：`promote_run` 只能把
...
- [x] **内容指纹，不只是配置值**（codex #427 r7 P1）：store / 日历 artifact 被
      ...
      **晋升侧同样要两次，不能只在入口复核一次**（codex #427 r15 P1）：晋升自己
      也要重建面板并求值，那段窗口无人看守 —— 期间刷新会让结果来自混合/新字节，
      而记录的身份仍然对得上。晋升须在**读面板前**取一次、**求值后且
      `target_dir.mkdir()` 之前**再取一次，不符即拒 —— 拒绝落在碰生产之前
      （与「拒绝早于 mkdir」同一条边界）。
- [x] **生产物化边界:机器可执行的拒绝**（codex #427 r6 P1）：
      ...
      **拒绝点必须落在写盘方，且早于 `target_dir.mkdir()`**（codex #427 r7/r8 P1）：
      ...
      纵深防御保留，但不能替代写盘方的拒绝。
- [x] `group_resolver` 参数今天恒传 `None`（PIT 行业 artifact 属后续 change）；
      签名与文档写明"绝不以当前快照兜底"。
- [ ] 性能：先测全历史 × CSI800 的墙钟时间；若逐日 Python 循环过慢，改为按
      `available_from_trade_date` 的 `merge_asof` 向量化 —— **但等价性须由 (i) 的
      抽样一致性断言守住**（两种实现必须给出同一面板）。
      **部分证据（链路验证批次 E001，2026-08-17）——不足以关闭本项**：
      窗 2021-01-01..2022-12-31，面板 (485, 964)、22 财报终端；面板构建
      **边际 0.86s/日**，一次性装载 45.3s。
      **证明了**：在 2 年 / 485 交易日 / 22 终端这一档上，逐日 Python 循环不是
      瓶颈（冷热分离后单日成本在毫秒量级）。
      **没证明**：本项要求的是**全历史** × CSI800。战役 IS 窗本身就是 4 年
      （2019-01-01..2022-12-31）、34 终端，是实测规模的 2 倍窗 × 1.5 倍终端；
      全历史更远超此。边际成本是否随窗长保持线性、一次性装载是否随历史长度
      恶化，**都还没有数**。在拿到全历史读数之前不得关闭本项，也不得据此断言
      `merge_asof` 逃生口不需要动用。
      run `20260817T133006-20260817-73b31a09`。

## 四件套防线（与桥同批落地，不可后补）

- [x] (i) `tests/logic/research/test_fundamental_panel_pit.py`：合成小店属性测试 —
      **该期公告前不被服务**（继续服务**胜者** = 该日**已可用**的最新报告期，
      没有任何已可用期时才 NA，而**不是**一律把 cell 置 NA；且可用日之后也只有
      在该期**本身是胜者**时才服务它 —— 更晚的报告期若已可用则继续服务后者，
      「可用」只是**有资格参选**，不等于**当选**，codex #427 r23 P1
      —— codex #427 r18 P1：把"新申报未到"写成"cell 为 NA"会在每次申报前后制造
      人为缺口，同时移动覆盖率与因子值）/ 严格次日起该期**才有资格参选**
      （是胜者才被服务）/ 公告日当天服务已可用期中的**胜者** / **无任何已可用期时
      值与证据同为 NA;而胜者对该字段无值时值为 NA、证据仍是该记录的可用日** / restated 期**解析为** original（版本选择决定一个期取哪个值，
      **不决定服务哪个期**;该期须本身是胜者才会被服务，codex #427 r25 P1）/
      missing-stays-missing（禁的是**填补**缺失值，不是禁止服务真实可用的旧期）；
      抽样 + 边界日（公告日、前一日、后首个交易日）核对
      `panel[T] == view.as_of(T)`。
- [x] (ii) `tests/logic/research/test_fundamental_panel_canaries.py` —— **金丝雀已按
...
- [x] (iii) `scripts/research/fundamental_ann_shift_sensitivity.py`：公告日整体后移
      ...
      须先核实相关性，而相关性 = **某个采样日上"胜出记录"发生了变化**
      （codex #427 r19 P2 → r22 P1 修正）：对每个采样交易日按 canonical as-of 规则
      （disclosure-of-record 归约后、可用日 <= 该日的**最新** report_period）在源侧
      算两个胜者 —— `W_base`（原可用日）与 `W_shift`（平移后可用日）；两者不同的
      采样日即相关。
      **按"单条披露的区间是否跨过采样日"来判是错的**（codex #427 r22 P1）：view 每日
      只服务**胜者**，当两条 disclosure-of-record 期的平移区间重叠（例如年报与一季报
      同日可用），逐条判会把两条都标为相关，而基线**从未服务过**其中的年报那条 ——
      再去断言"基线必须服务每条跨越记录"就会拒绝一个正确实现。比较胜者问的正是
      serve-rule 回答的那个问题。**该判定必须只看源数据**（store 的披露日、平移量、
      被请求字段、采样日历），**绝不查重建出来的面板**（codex #427 r20 P1）——
      用"基线服务而平移后不服务"来定义相关性是**循环论证**：对公告日不敏感的
      构建器（按报告期取值）平移后照样服务同一条披露，于是相关性永远建立不起来、
      诊断永远 INCONCLUSIVE，那条"哈希不变即 REFUSE"**永远不会触发** —— 防线正好
      对它要抓的缺陷失效。面板输出只用于**断言本身**，不用于决定断言是否适用。**"在某个采样日被服务"还不够** —— 若该披露的原可用日与平移后
      可用日落在同一批采样日的同一侧（都早于首个采样日、或夹在同两个稀疏采样日
      之间），它确实被服务且相关，却没有任何采样 cell 会动，无条件规则又会误拒
      正确实现。无跨越则报 **INCONCLUSIVE**（提示扩大采样或改用确定性 fixture），
      **不得**判 REFUSE。
      ...
      **相关性只能建立在 disclosure-of-record 选择之后的源行上**（codex #427 r21 P2）：
      逐 `report_period` 先做 canonical 选择 —— 优先 `update_flag=0`；该期**没有**
      uf0 行时取 `update_flag=1`；选定版本内取**最早公告**的那条。
      被丢弃的**只是未被选中的行**：与首选 uf0 并存的那条 uf1、以及已选版本的后续
      再公告。**uf1 不是一律排除**（codex #427 r25 P1）—— 只有 uf1 行的期，那条
      uf1 就是它的 disclosure of record、是**会被服务**的；把这类期丢掉会让近年
      申报整体从相关性中消失，反而让对公告日不敏感的构建器拿到 INCONCLUSIVE 脱身。
      让未选中的行移动胜者会要求正确面板做出它本不该做的移动（**误拒**），丢掉被
      选中的行则会**解除诊断的武装**（**漏放**）—— 两个方向都靠"与 view 完全同款
      的选择"避免。
      确认相关性之后，**断言落在"被服务的是哪条记录"上，不是落在哈希上**
      （codex #427 r21 P1）：在**胜者发生变化**的每个采样日，基线须服务 `W_base`、
      平移后的重建须服务 `W_shift`（无可用记录则 NA）—— 被服务的报告期/记录
      provenance 必须**换人**，不换即 **REFUSE**（报告"公告日未被消费"）。
      胜者未变的日期**不作断言**。
      只比"值+证据"哈希**挡不住**：按报告期取值的构建器可以继续服务同一条（仍属未来的）
      披露与其值，只把该记录**平移后**的 `available_from` 抄进证据 —— 证据字节变了、
      哈希变了，而值的选择依旧对公告日盲目。**证据可以抄，"服务了哪一期"抄不了。**
      值在平移前后**允许相等**（延迟申报可能重复上期同值、或两期皆 NA），故不要求
      值不等；哈希可作为记录保留，但不作判据。**IC 断言只挂在确定性 fixture 上**（该 fixture 构造成
...
- [x] 确认改动面与提案一致：`grammar.py`（终端注册 + 名字映射 + prior 期终端/算子）、
...
- [x] 确认 D5 gate 与 `test_financial_pit_view_isolation.py` 均照原样通过
      （**不改签**）—— 终端注册不引入 qlib/PIT import，桥仍在 `src/research/` 内。
- [x] ruff + mypy --strict + 治理全套 + 新增测试全绿。
- [x] 用起步三因子（① GP/A、③ 资产增长、② C3 纯 BS 应计 —— CSI800 覆盖率已实测全绿）
      跑通"面板化 → GP → 边际贡献"最小链路，**只为验证链路与防线，不做因子裁决**
      （裁决属后续预注册战役）。
      **实测（E001）**：mine 1025.4s、starter-check 482.0s；三个冻结起步因子确定性
      求值全部产出观测 —— coverage 0.9928 / 0.9956 / 0.9495，日换手
      0.0055 / 0.0049 / 0.0066，rank_ic_mean −0.011862950049470897 /
      −0.008976871647367497 / −0.007631585807496477。**不构成因子裁决**：三个读数
      都是微负且无预注册门，按协议只能读作"链路成立"，不能读作方向证据。
