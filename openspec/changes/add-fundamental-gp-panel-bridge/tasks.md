...
- [ ] (i) `tests/logic/research/test_fundamental_panel_pit.py`：合成小店属性测试 —
      **该期公告前不被服务**（继续服务**胜者** = 该日**已可用**的最新报告期，
      ...
      值与证据同为 NA;而胜者对该字段无值时值为 NA、证据仍是该记录的可用日** / restated 期服务 original /
...
- [ ] (iii) `scripts/research/fundamental_ann_shift_sensitivity.py`：公告日整体后移
      ...
      **相关性只能建立在 disclosure-of-record 选择之后的源行上**（codex #427 r21 P2）：
      原始行含再公告与 `update_flag=1` 重述，view 从不服务它们；若让这类行确立相关性，
      就会要求一份**正确**面板做出它本不该做的移动，诊断反而因构建器遵守 serve-rule
      而拒绝它。
