...
- [ ] (iii) `scripts/research/fundamental_ann_shift_sensitivity.py`：公告日整体后移
      ...
      须先核实相关性，而相关性 = **平移跨过了某个采样日**（codex #427 r19 P2）：
      某条被请求字段的披露，其**原**可用日与**平移后**可用日之间（半开区间）
      至少含一个采样交易日。**该判定必须只看源数据**（store 的披露日、平移量、
      ...
      **相关性只能建立在 disclosure-of-record 选择之后的源行上**（codex #427 r21 P2）：
      ...
      （codex #427 r21 P1）：在跨越区间内的每个采样日，平移后的重建须服务**前一条**
      披露（无则 NA），基线服务跨越的那条 —— 被服务的报告期/记录 provenance 必须
      **换人**，不换即 **REFUSE**（报告"公告日未被消费"）。
