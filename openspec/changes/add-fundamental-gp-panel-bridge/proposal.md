...
* **view 的公开 provenance 响应**（`src/research/financial_pit_view.py` 非零改动）：
  ...
  值为 NA 时**仍带其可用日**；证据为 NA 的条件是**该日无任何记录被服务**（即尚无任何期
  **可用**）—— **不是**「尚无任何已公告记录」：可用日严格晚于公告日，公告当天记录已公告
  却无物可服务，证据同样必须为 NA（codex #427 r3 P2 / r24 P1）—— 否则"两期皆 NA"的字段在 (iii) 平移下值与证据都不变，会误拒正确实现。
...
* **(i) as-of 断言** — 返回前自检的键**落在证据上而非值上**（codex #427 r4 P2）：
  ...
  属性层：合成小店上断言"该期 `available_from` 前**不被服务**（继续服务**胜者** =
  该日已可用的最新报告期，无任何已可用期时才 NA，而**不是**一律把 cell 置 NA；
  可用日之后也只有在该期**本身是胜者**时才服务它 —— 更晚的报告期若已可用则继续服务
  后者，「可用」只是有资格参选、不等于当选，codex #427 r23 P1 —— codex #427 r18 P1：写成置 NA 会在每次申报前后
  制造人为缺口，同时移动覆盖率与因子值）、**自该日起仅当该期是胜者时**才等于其
  disclosure-of-record 值、
  **公告日当天服务已可用期中的胜者**（严格次日该期才有资格参选）、
  **无任何已可用期时值与证据同为 NA;胜者对该字段无值时值为 NA、证据仍是该记录的
  可用日**（codex #427 r24 P1/P2）、
  restated 期**解析为** original（版本选择决定一个期取哪个值、不决定服务哪个期；
  该期须本身是胜者才会被服务，codex #427 r25 P1）"；运行时层：桥输出证据帧后自检 `(available_from <= T).all()`，并对抽样日期
...
* **(iii) 公告日平移敏感性（最锋利的一把刀；断言按 codex #427 P2 分层）** — 把店内有效
  ...
  - **先确认相关性**（codex #427 r9/r19 P2）：被平移的披露未必触及所请求的字段/端点/
    ...
    都不变，无条件规则会**误拒正确实现**。相关性的正确定义是**某个采样日上"胜出记录"变了**（r19 的"跨采样日"经 r22 P1 修正）：
    按 canonical as-of 规则在源侧算 `W_base`（原可用日）与 `W_shift`（平移后），
    两者不同的采样日即相关。**逐条披露判区间是错的** —— view 每日只服务胜者，
    两条 disclosure-of-record 期的平移区间重叠时（年报与一季报同日可用）逐条判会把
    两条都标相关，而基线从未服务过年报那条，断言反会拒绝正确实现。
  ...
  - **确认相关性后的断言：被服务的记录必须换人**（codex #427 r21 P1）。在跨越区间内的
    **胜者发生变化的**每个采样日，基线须服务 `W_base`、平移后的重建须服务 `W_shift`
    （无可用记录则 NA）——
  ...
  - **相关性只能建立在 disclosure-of-record 选择之后的源行上**（codex #427 r21 P2）：
    逐期先做 canonical 选择（优先 uf0；无 uf0 则取 uf1；选定版本内取最早公告）。
    被丢弃的**只是未被选中的行** —— 与首选 uf0 并存的 uf1、已选版本的后续再公告。
    **uf1 不是一律排除**（codex #427 r25 P1）：只有 uf1 行的期，那条 uf1 就是它的
    disclosure of record、会被服务；丢掉这类期会让近年申报从相关性中消失，反而给
    公告日不敏感的构建器一个 INCONCLUSIVE 的出口。**误拒**与**漏放**两个方向，
    都靠"与 view 完全同款的选择"避免。
