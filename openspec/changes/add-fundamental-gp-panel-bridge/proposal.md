...
却在新增 OOS 日期上发散。扩窗晋升**必须**由①实现摘要绑定 —— ②在扩窗下**不可用**
（codex #427 r16 P1）：`validation_end_date` 只在晋升时给出，挖掘只可能存下原窗的输出
摘要，拿有效窗摘要比它必然不等；而哈希"被晋升工厂自己在有效窗上的输出"等于**自己给
自己发证**。唯一另一条路是由**独立可信的过程**在晋升前把扩窗及其预期输出摘要落盘。
...
* **(iii) 公告日平移敏感性（最锋利的一把刀；断言按 codex #427 P2 分层）** — 把店内有效
  ...
  - **先确认相关性**（codex #427 r9 P2）：被平移的披露未必触及所请求的字段/端点/
    instrument —— 只取 revenue 的面板本就该无视一份纯资产负债表申报，此时值与证据
    都不变，无条件规则会**误拒正确实现**。须先核实"至少有一条被平移的披露确实被该
    面板在某个采样日服务"；没有则报 **INCONCLUSIVE**（扩大采样或改用确定性
    fixture），**不得**判 REFUSE —— 诊断不出结论不等于实现有罪。
  - **确认相关性后的无条件断言**：**值 + 证据一起**的哈希必须改变。**不变 ⇒ 面板化根本没消费公告日
