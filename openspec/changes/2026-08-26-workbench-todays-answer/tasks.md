...
## 第十八轮（3×P2：修一划二，用户裁决选项 A）

- [x] P2b 节奏双字段 both-or-neither（write_outputs 同一守卫块同写两键）：
      只带其一=需核查——缺 next 键时 hold_state 静默补 None，头卡会报
      「HOLD·未记录」；显式 null（无锚）与缺键分清。变异 2f 咬住
- [ ] **划界不修（用户裁决）**：P2a（读侧拿 bundle 日历验 entry=as_of 后
      首会话）与 P2c（读侧重跑 is_rebalance_day 验 rebalance_day 标记）
      ——两条都要求读侧**重新推导产出器的日历语义**：合成层从此消费完整
      交易日历、维护第二份可漂移的推导，正是本 change 全程遵守（且 codex
      前十七轮反复援引）的「零自造判定/#461 另写一份三决策全错」反模式。
      工件字段即产出器对该推导的署名；「产出器算错」由产出器侧测试与五门
...
- [x] 写侧节奏不变式执法：True+None / True+异日是 recommend() 不可达组合
      （next_rebalance_date(as_of) 在再平衡日恒返 as_of），但 write_outputs
      放行、读侧核验判损坏——仓与自己打架（矛盾是我第十九轮引入读侧不变
      式时埋的）。序列化边界拒绝 + 原 True+None 用例迁移到 False（None 锚
      = HOLD 日现象）+ 执法用例两形态；变异 2f 咬住
- [ ] **划界重申**：HOLD 锚对 canonical 日历表的校验 = 第十八轮已划掉的
      「读侧消费日历复推产出器语义」同族重现（第一次）。PR 回帖重申边界；
      若该族再现即确认不收敛，转 #463 式终轮划线呈裁决

## 第二十五轮（2×P2：执法晚于 I/O + 显式 null nonce 借 legacy 道）

- [x] 写侧两道执法（nonce 键/型 + 节奏不变式）全部上提到**任何文件 I/O
      之前**——原节奏执法在 CSV 落盘后才抛，失败运行留下新 CSV 配旧/缺
      JSON 的半成品输出
- [x] tag×显式 null nonce 双侧拒绝：tag 非空证明 stamp 存在 ⇒ built_at
      当时必然可得——键在场写 null 是产出器产不出的组合，当 legacy 放行
      会在同日历原地重建后跳过 nonce 比对。缺键 legacy（前 nonce 时代）
      照旧放行，专用断言防误伤
- [x] 变异三发（去写侧组合拒 1f/执法回落 2f/去读侧组合拒 1f）全咬
