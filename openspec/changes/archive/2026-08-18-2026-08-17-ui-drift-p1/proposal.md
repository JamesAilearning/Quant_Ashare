...
条是相对路径，按 CWD 解析会随启动目录变答案）。判据**逐行只做词法运算**
——起初它逐行 `resolve()`，那会走符号链接、真碰盘：本机 3527 行实测每次
渲染 **771 ms**；改成词法锚定 + 两个根只解析一次后是 **23 ms**（codex
#444 r7 与自审并行抓到同一条）。解析型守卫留在**真正读文件**的边界上。
