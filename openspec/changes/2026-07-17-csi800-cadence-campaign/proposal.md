...
- **DP-3 主判据（预注册双条件，AND）**：
  ...
  **N1 基线机器钉死（codex #374 r1）**：50% 比较的 N1 毛值不取自
  任何可编辑文档——#373 已提交 v2 配对工件钉住 N1 base 的逐折
  fold report 哈希，毛值本体在被钉的 fold report 内；PR-B 以 v3
  重生成 N1 工件（新增逐折毛值，v2 已钉哈希逐字段不变的治理断言），
  比较工装仅消费已提交 v3 工件、缺失/失配/覆盖不全一律拒绝；N1
  run 目录保持完好直至战役收束，丢失即 fail-closed。
