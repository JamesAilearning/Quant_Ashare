...
- **DP-3 主判据（预注册双条件，AND）**：
  ...
  **N1 基线机器钉死（codex #374 r1，r7/r8 修订终态）**：50% 比较的
  N1 毛值不取自可编辑文档、不依赖单机 run 目录、且值的来源在
  fresh checkout 可复验——PR-B 将 N1 双侧 46 个 **fold report 源
  文件本体**（~1.1 MB）提交至钉死证据目录（`.gitattributes -text`
  字节保真），治理测试逐折断言源文件 sha256 == 已提交 v2 工件所钉
  `fold_report_sha256`（CI 端到端可验），比较工装直接从哈希验证过
  的已提交源读毛值、缺失/断链/覆盖不全一律拒绝。**比较臂钉死
  （r7）**：conservative-to-conservative，且各 pair 内双臂毛均值
  相对差 ≤5% 否则 fail-closed。
...
- **DP-5 晋升 attestation 前置（本 change 唯一 runtime 触点；r4 修订
  ——侧车是唯一晋升权威）**：
  ...
  2. pair 工件升 **v3**：参照 run 作为第三方入证（run_id/config_sha256/
     report_sha256/fold_report_sha256 同款四件）+ 逐折毛值入档；
  3. **attach 验全链摘要但内嵌资格恒 false（非权威）**；独立
     **certify** 步骤验证 pair v3 字节==主线锚（`origin/main` 可达
     commit，HEAD 一致不充分）+ 经证据锚读全部 N5 源证据字节 +
     N1 侧经 N1 主线锚读取 + 全摘要链 + 五 veto/主判据，全过产出
     verdict 侧车（被锚 pair digest + **三锚 commit id：pair +
     N5 证据 + N1** + 判定）；证据或 N1 未主线锚定时 certify 拒绝
     ——`producer_digest_certified` 与晋升仅以"已提交侧车 + digest
     与已提交 pair 一致 + 三锚可复验"组合形态成立，顺序
     run→attach→pair 提交→源证据提交→certify→侧车提交→晋升不可
     倒置。
  三件全绿并入 main 之后才允许点火（沿 #368 "护栏先行"纪律）。
- **DP-6 敏感带与约束不动**：20 bps 预注册值、敏感带成对构造、
  `campaign_risk_constraints_v1`、金融不排除——全部沿 #368/#372 原样。

- **R1 约束作用域修订（选项 A，2026-07-18 操作人签，结果盲）**：首发
  N5 点火中 csi800 双档同折同票同日被 `max_per_name` RAISE 击落
  （SH601127 @2021-05-12，5.04%/5.05% vs 5.00%——单日 0.04-0.05pp
  漂移超幅）。定性：约束对 positions map 逐日检查在两种节奏下**语义
  不等价**——N=1 的每日再平衡天天把权重重置回目标，检查只见再平衡后
  状态；N>1 时未再平衡日的市场漂移也被检查，同一数字更严，破坏"同
  配置可比"前提。修订：campaign 约束仅在**再平衡生效日**（thinned
  stamp + signal_to_execution_lag 总滞后）校验——以**显式 opt-in**
  配置字段 `risk_constraint_scope: "rebalance_days"` 承载（canonical
  默认 `all_days` 对所有调用者保持全图合约不变，codex #378 r3），
  campaign preset 显式声明、veto④ 双重强制（config 声明+逐折
  provenance 披露）；5% 数字、RAISE 模式、全部 veto 数字不动；
  N=1 路径逐字节不变。**结果盲声明**：修订决策时
  三 run 的业绩数字一个未读（日志仅查 FAILED 行），与 #372 同级正当
  性；既有三发作废，修订合并后重跑。
...
- **MODIFIED capability `v2-csi800-expansion-guards`**：
  ...
  3. N5 降换手战役预注册（DP-1/2/3/4，研究纪律层）：三 preset、
     ...
     恰 {slippage_bps, output_dir} 等沿用；N5 vs N1 同角色 preset
     恰差 {rebalance_cadence_days, rebalance_phase, rebalance_anchor,
     output_dir}——N1 preset 不含 cadence 三键（继承默认），N5 显式
     写死三键，故 raw diff 为三 cadence 键 + output_dir，codex r12）。
- NO 官方超额语义变更：attestation 是工件字段追加；cadence 用 7a
  已有机器；判据是研究纪律层。

## Dependencies

本 change 的 specs delta **MODIFIED** `v2-csi800-expansion-guards`，
该 capability 目前仅存在于未归档的
`2026-07-16-csi800-antiinflation-guards` change 中、尚未物化进
`openspec/specs/`（codex #374 r17）。故 PR-A 的**第一步**是归档该
前置 change（连同 `2026-07-16-per-universe-canonical-benchmark`），
本 change 的 delta SHALL 在归档完成、canonical spec 存在之后才
应用/校验；提案签署不受此影响（签的是决策账与 delta 文本）。
