...
- **DP-3 主判据（预注册双条件，AND）**：
  ...
  **N1 基线机器钉死（codex #374 r1，r7 修订）**：50% 比较的 N1 毛值
  不取自任何可编辑文档，也不依赖单机 run 目录——PR-B 在仍持有 N1
  run 目录的机器上抽取并**提交 N1 毛值证据工件**（逐折毛超额双侧
  + 抽取时验证过的 fold_report_sha256 + 源 pair v2 digest；治理测试
  断言两个已提交工件间哈希一致，CI 无需 run 目录即可执行），比较
  工装仅消费已提交证据、缺失/断链/覆盖不全一律拒绝。**比较臂钉死
