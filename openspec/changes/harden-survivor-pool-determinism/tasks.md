...
- [x] `evaluator.max_abs_corr_with_skips`：返回 (max, 不可比对数)；
      `max_abs_corr` 签名与返回类型不变、逐字委托。
      `filter_correlated` 见到任何不可比配对即拒。
      **注意：三处调用方并非零影响**（codex #448 r6 P2 —— 这句是
      r5 之前写的，已被 r5 的修复推翻）。相关性现在先按 `np.isfinite`
      过滤再判重叠与计算：含 `inf` 但有限观测仍足量的配对，此前贡献
      `0.0`（一个 inf cell 抹掉真实强相关），现在照常算出真实相关。
      因此 **GP novelty penalty 与 `FactorPool.correlation_with` 的取值
      会变**（变准，但确实是变化），池过滤也会多剔本就该剔的。
      T2-3 的原保证（永不返回非有限值）原样成立。
      回归：稀疏对 (0.0, 1) vs 稠密对 (>0.99, 0)；含单个 inf 且有限
      重叠充足 → 不判不可比且算出真实相关；有限观测不足 → 仍退 0.0。
