...
- [x] `build_manifest(results, holes, end_date, *, now=None)` — injectable
      timestamp (Phase 2 value-injection pattern; default `datetime.now(tz=utc)`).
- [x] `read_manifest` (missing → None / unknown schema_version → fail-loud) +
      `write_manifest` (atomic temp + `os.replace`) + `clear_manifest`.
- [x] `merge_manifest(prev, current)` — self-heal: re-resolve each ran endpoint
      ...
      accumulated, coverage advances); preserve non-run endpoints untouched.
- [x] `01_fetch_tushare.main`: on the completed-run path (not `--dry-run`), read
      prev → build current → merge → write atomically.
...
- [x] READ: missing → fresh (None, no error); unknown / missing `schema_version`
      and malformed JSON → `FetchManifestError`.
- [x] MERGE: self-healed hole dropped; un-healed hole kept with attempts
      accumulated; coverage advances and never regresses.
...
- [x] INTEGRATION: through `01.main` twice — run 1 records the hole (exit 3),
      run 2 (unit now succeeds) self-heals it (exit 0).

## 3. Verification
- [x] `pytest tests/data_pipeline/test_fetch_manifest.py` green (18 tests).
- [x] Full fast suite green (2332 passed); `ruff` + `mypy --strict` clean.
