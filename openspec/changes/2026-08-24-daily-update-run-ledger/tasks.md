...
外时区照样过。补齐：与 `datetime.isoformat()` 精确回环 + 写入侧固定的
+08:00（边界戳的时区也顺带钉上，同一「写侧产不出」原则）。变异揭出我的
+00:00 用例被「结束早于开始」检查**遮蔽**（UTC 20:30 = 东八 28:30）——时区
检查一度不可测；换成不触发排序的 UTC 时刻后 CY'/CZ' 咬住。
