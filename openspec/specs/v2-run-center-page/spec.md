# v2-run-center-page Specification

## Purpose
TBD - created by archiving change 2026-08-16-ui-run-center. Update Purpose after archive.
## Requirements
...
轮询范围 SHALL 再多覆盖一段：**刚点过启动、子进程尚未写出 running 记录
的那段窗口**。否则从空闲页启动之后，页面会一直停在启动前的状态直到操作
人手动刷新——他会以为没启动成功而重复点击。该窗口 SHALL **有界**（默认
5 分钟），否则一次秒退的启动（撞单飞锁 exit 17）会让页面永远轮询下去。
到期判定 SHALL 由轮询片段**自身**执行并触发整页重绘：片段计时只重跑片
段，主脚本里算出的窗口判断在片段注册之后不会再被求值。退休条件 SHALL 是
「出现**新鲜**的 running 记录」而非「出现 running 记录」——带着一条陈旧
记录去做恢复性补跑时，按旧记录清掉标记会让轮询立刻停摆。

日志尾部 SHALL 逐行解码（UTF-8 优先、**仅在解码失败时**回退 GBK、再失败
按替换符降级）：这条共享日志有两个写入者，行不跨进程，所以逐行而非整块。

编码在**源头**钉死：本页启动的运行经 `utf8_child_env` 钉 UTF-8；计划任务的
包装脚本只有在**它自己**补过 `PYTHONIOENCODING=utf-8`（模板见
`docs/runbook_daily_update_scheduling.md`）之后才钉住。仓库对既有部署没有
强制力，所以按旧模板建的调度任务**今天仍在写** cp936 新行——混编码既是历史
遗留，也可能是当下现象。运维文档与**页面文案**都 SHALL 指示先核对 `.bat` 里
的那一行，且 MUST NOT 把编码钉说成无条件已生效的事实。

读侧 MUST NOT 按字符区段猜测编码：那会把合法的新行改写坏（provider 路径里
一个 `José` 被改成 `Jos茅`），是拿新行的损坏换历史行的修复。代价是字节恰好
也是合法 UTF-8 的那类历史行无法还原，页面 SHALL 明示这一边界。
...
#### Scenario: 刚启动、记录尚未出现时也在轮询

- **GIVEN** 操作人刚点过启动，子进程还没写出 running 记录
- **WHEN** 页面驻留
- **THEN** 页面照常按固定间隔重读状态工件
- **AND** 该等待窗有界；窗口到期由轮询片段自身判定并触发整页重绘
- **AND** 只有出现**新鲜**的 running 记录才让等待标记退休
